SUBROUTINE DTGSYX(TRANS,M,N,A,LDA,B,LDB,C,LDC,D,LDD,X,LDX,WORK,SCALE,INFO)
!
!     PURPOSE
!
...
!     overflow in X.
!
!     In contrast to DTGSYX2 this variant uses a modification of the
!     Generalized Schur(GS) algorithm by Kagström and Westin. If the
!     matrices A and B are upper quasitriangular  and  the matrices
...
IF ( TEQN .EQ. 1 ) THEN
    ...
    DO WHILE ( I <= M )
        ...
        DO WHILE ( J <= N )
            ...
            IF (JE < N ) THEN
                CALL DGEMM('N','N', IB, N-JE, JB, MONE, WORK(IS,JS), M , D(JS,JE+1), LDD, &
                    & ONE, X(IS, JE+1), LDX)
    ...
    ! Condition number of B and C
    EMIN = ABS(B(1,1))
    EMAX = ABS(B(1,1))
    DO I = 1,N
        IF ( EMIN > ABS(B(I,I))) THEN
            EMIN = ABS(B(I,I))
        END IF
        IF ( EMAX < ABS(B(I,I))) THEN
            EMAX = ABS(B(I,I))
        END IF
    END DO
    IF ( EMIN .EQ. 0.0D0) THEN
        EMIN = 1d0
    END IF
    CONDB = EMAX / EMIN
    ...
    EMAX = ABS(C(1,1))
    DO I = 1,M
        IF ( EMIN > ABS(C(I,I))) THEN
            EMIN = ABS(C(I,I))
        END IF
        IF ( EMAX < ABS(C(I,I))) THEN
            EMAX = ABS(C(I,I))
        END IF
    END DO
    IF ( EMIN .EQ. 0.0D0) THEN
    ...
    ! Solve with the Better conditioned system
    IF ( CONDB > CONDC ) THEN
        CALL DLACPY('A', M,N,WORK,M,X,LDX)
        CALL DTRSM('L','U','T','N',M,N,ONE,C,LDC,X,LDX)
    ELSE
        CALL DTRSM('R','U','N','N',M,N,ONE,B,LDB,X,LDX)
    END IF
ELSE
    ...
    DO WHILE ( I >= 1 )
        ...
        DO WHILE (J >=1)
            IF ( J > 1 .AND. (D(J,J-1) .NE. 0.0d0)) THEN
                JS = J -1
                JE = J
                J = J - 2
                JB = 2
            ...
            IF ( JS >  1 ) THEN
                CALL DGEMM('N','T',IB,JS-1,JB,MONE,WORK(IS,JS),M,D(1,JS),LDD,ONE, X(IS,1),LDX)
    ...
    ELSE
        CALL DTRSM('R','U','T','N',M,N,ONE,B,LDD,X,LDX)
    END IF
END IF
END SUBROUTINE

SUBROUTINE DTGSTN(TRANS,ISOLVE,NB,N,A,LDA, E, LDE, X, LDX, SCALE, WORK, IWORK,INFO)
!     PURPOSE
!
...
!             Only ISOLVE .EQ. 3 allows to compute the scaling factor
!             SCALE.
!
!    NB       INTEGER
...
!     *********************************
 ...
 EXTERNAL DTGSYY,DTGSYY2,DCOPY,DGEMM,DLACPY
 ...
 IF ( (.NOT. LSAME(TRANS,'N')) .AND. (.NOT.LSAME(TRANS,'T'))) THEN
         INFO = -1
 ELSE IF ( ABS(ISOLVE) < 1 .OR. ABS(ISOLVE) > 2) THEN
         INFO = -2
 ELSE IF ( NB < 1) THEN
         INFO = -3
 ELSE IF ( N < 0 ) THEN
     ...
     DO WHILE (KH .NE. N)
           ...
           DO WHILE ( LH .NE. N )
                ...
                IF ( LH > 1 ) THEN
                    ...
                    CALL DGEMM( 'T', 'N', LH-KL+1, LB, KB, ONE, E(KL,KL), LDE, WORK, KB, ONE, X(KL,LL), LDX )

                END IF

                ! Solve inner System
                IF( NB .GT. 1 .AND. INNERSTEIN .AND. KL .EQ.LL) THEN
                    I = 1
                    CALL DTGSTNX("N",IISOLVE,I,KB,A(KL,KL),LDA, E(KL,KL), LDE, X(KL,KL), LDX, SCAL, WORK, IWORK,IINFO)
                    IF (SCAL .NE. ONE ) THEN
                ...
                ELSE
                    IF ( IISOLVE .EQ. 1 ) THEN
                        CALL DTGSYY2("N",KB,LB,A(KL,KL),LDA,A(LL,LL),LDA,E(KL,KL),LDE,E(LL,LL),LDE, &
                            & X(KL,LL), LDX, WORK,IWORK,IINFO)

                    ELSE
                        CALL DTGSYY("N",KB,LB,A(KL,KL),LDA,A(LL,LL),LDA,E(KL,KL),LDE,E(LL,LL),LDE, &
                ...
                IF ( KL < LL ) THEN
                    ...
                    CALL DGEMM("T","N",LH-KH,LB,KB,ONE,E(KL,KH+1),LDE,WORK,KB,ONE,X(KH+1,LL), LDX)
                END IF
           END DO
   END DO
 ELSE
   ...
   DO WHILE ( LL .NE. 1 )
        ...
        DO WHILE ( KL .NE. 1)
           ...
           IF ( KH < N ) THEN
                  ...
                  CALL DGEMM( 'N', 'T', KB, LH-KL+1, LB, ONE, WORK,KB,E(KL,LL), LDE, ONE, X(KL,KL), LDX )
           END IF

           IF ( NB .GT. 1 .AND. INNERSTEIN .AND. KL.EQ.LL) THEN
               I = 1
               CALL DTGSTNX("T",IISOLVE,I,KB,A(KL,KL),LDA, E(KL,KL), LDE, X(KL,KL), LDX, SCAL, WORK, IWORK,IINFO)
               IF (SCAL .NE. ONE ) THEN
                   CALL DLACPY("All",KB,LB,X(KL,LL),LDX,WORK,KB)
                   DO I = 1,N
                       CALL DSCAL(N,SCAL,X(1,I),IONE)
           ...
           ELSE
               IF (IISOLVE .EQ. 1 ) THEN
                   CALL DTGSYY2("T",KB,LB,A(KL,KL),LDA,A(LL,LL),LDA,E(KL,KL),LDE,E(LL,LL),LDE, &
                       & X(KL,LL), LDX, WORK,IWORK,IINFO)
               ELSE
                   CALL DTGSYY("T",KB,LB,A(KL,KL),LDA,A(LL,LL),LDA,E(KL,KL),LDE,E(LL,LL),LDE, &
                       & X(KL,LL), LDX, WORK,SCAL,IINFO)
                   IF (SCAL .NE. ONE ) THEN
                       CALL DLACPY("All",KB,LB,X(KL,LL),LDX,WORK,KB)
                       DO I = 1,N
           ...
           IF ( KL < LL ) THEN
                  ...
                  CALL DGEMM( 'N', 'T', KB, LL-KL, LB, ONE, WORK, KB,E(KL,LL), LDE, ONE, X(KL,KL), LDX )
           END IF
        END DO
   END DO
 END IF
...
! Dummy to override the recursive call
SUBROUTINE DTGSTNX(TRANS,ISOLVE,NB,N,A,LDA, E, LDE, X, LDX, SCALE, WORK, IWORK,INFO)
 CHARACTER      TRANS
 INTEGER        ISOLVE,NB,N,LDA,LDE,LDX,INFO
 INTEGER        IWORK(*)
 DOUBLE PRECISION A(LDA,*), E(LDE,*), X(LDX,*)
 DOUBLE PRECISION WORK(*)
 DOUBLE PRECISION SCALE
 CALL DTGSTN(TRANS,ISOLVE,NB,N,A,LDA, E, LDE, X, LDX, SCALE, WORK, IWORK,INFO)
END SUBROUTINE

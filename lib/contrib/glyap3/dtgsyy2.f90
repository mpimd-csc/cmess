SUBROUTINE DTGSYY2(TRANS,M,N,A,LDA,B,LDB,C,LDC,D,LDD,X,LDX,WORK,IWORK,INFO)
!
!     PURPOSE
!
...
!
!     In contrast to DTGSYY this variant uses a modification of the
...
!     by DTGSYY.
!     If the matrices A and D are upper quasitriangular  and  the matrices
...
!
!     WORK    DOUBLE PRECISION array, dimension (4*M*(M+1))
...
EXTERNAL DLASET,DGEMM,DGETC2,DTRSM,DLACPY,DGESC2X
...
IF (TEQN .EQ. 1 ) THEN
    ...
    K = 1
    DO WHILE ( K <= N)
        IF (((K .LT. N) .AND. (B(K+1,K) .EQ. ZERO)) .OR. (K .EQ. N)) THEN
        ...
        ELSE
            ...
            DO WHILE ( L <= 2*M  )
                IF ( (L+3 <= 2*M) .AND. ( WORK(L+2,L) .NE. ZERO) ) THEN
                    ! CALL DGETRF(4,4, WORK(L,L), 2*M, IWORK, IINFO)
                    ! CALL DGETRS("T", 4, 1, WORK(L,L), 2*M, IWORK, WORK(L,2*M+1),2*M,IINFO)
                    ...
                    L = L + 4
                ELSE
                    ! CALL DGETRF(2,2, WORK(L,L), 2*M, IWORK, IINFO)
                    ! CALL DGETRS("T", 2, 1, WORK(L,L), 2*M, IWORK, WORK(L,2*M+1),2*M,IINFO)
...
ELSE
    K = N
    DO WHILE ( K >= 1 )
        IF ( K .EQ. 1 .OR. (K > 1 .AND. B(K,K-1) .EQ. ZERO)) THEN
            ! WORK(1:M,1:M) = ZERO
            DO I = 1, M
                CALL DCOPY(M,A(1,I), IONE, WORK(1,I), IONE)
                CALL DSCAL(M,B(K,K), WORK(1,I),IONE)

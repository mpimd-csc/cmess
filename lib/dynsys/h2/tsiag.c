...
/**
 * @file lib/dynsys/h2/tsiag.c
 * @brief Generalized TSIA Algorithm.
 * @author @koehlerm
 */
...

{
    ...
    if ( !MESS_IS_DYNSYS_GLTI(orig)){
        MSG_ERROR("only generalized state space LTI systems allowed.\n");
    ...
    if ( usesigma == 1) {
        ...
        ret = __constructVWmat(A, NULL, NULL ,B,C,sigmaEX, Vk,Wk);  FUNCTION_FAILURE_HANDLE(ret, (ret!=0),  __constructVWmat);
    ...
    if (opt->calc_h2err == MESS_H2NORM_FULL ){
        ...
        ret = mess_h2_norm(orig,&h2orig);                   FUNCTION_FAILURE_HANDLE(ret,(ret!=0),mess_h2_norm_internal);
    ...
    // for ( i = 1; i<=maxit ; i++ ){
        ...
        ret = mess_direct_create_sylvester_sparsedense(A,NULL,E,ArT,sylv);          FUNCTION_FAILURE_HANDLE(ret, (ret!=0), mess_direct_create_sylvester_sparsedense);
        // solve
        ...
        if (opt->calc_h2err == MESS_H2NORM_FULL ){
            ret = mess_h2_error_internal(A,B,C,E,Ar,Br,Cr,NULL,&h2err);

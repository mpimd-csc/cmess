...
/**
 * @file lib/dynsys/bt/gslrsrm.c
 * @brief GS-LR-SRM balanced truncation.
 * @author @koehlerm
 */
...
/**
 * @brief Generalized low-rank square root balanced truncation for first order GLTI to LTI systems.
 ...
 *
 * The @ref mess_bt_gslrsrm function computes the left and the right projection matrix for
 * a balanced truncation reduced order model of a generalized state space system to a
 * standard state space system. \n
 * In order to get the reduced order LTI model from the output, you have to call the
 * \ref mess_dynsys_project_to_lti function with the computed  \f$ V \f$ and \f$ W \f$. \n
 * In the @ref mess_options.adi_res2_tol is given to set the cut off
 ...
 * If you do not want to choose the reduced order in a adaptive way so can specify a \c chooseorder function
 * to decide it in your own way. The default choose order function is \ref mess_bt_chooseorder_default.
 *
 * The algorithm is based on Algorithm 7.2 in \cite Saa09 .
 *
 *
 */
int mess_bt_gslrsrm(mess_dynsys sys, mess_bt_options btopt, mess_options adiopt, mess_matrix V, mess_matrix W, mess_bt_status status) {
    MSG_FNAME(__func__);
    mess_matrix A,B,C,E, ZB,ZC,ZCT, ZCZB, SC;
    ...
    if ( !MESS_IS_DYNSYS_GLTI(sys) ){
        MSG_ERROR("Only for generalized state space LTI systems\n");
    ...
    ret = mess_matrix_init(&ZCT); FUNCTION_FAILURE_HANDLE(ret,(ret!=0),mess_matrix_init);
    ret = mess_matrix_init(&ZCZB); FUNCTION_FAILURE_HANDLE(ret,(ret!=0),mess_matrix_init);
    ret = mess_matrix_init(&SC); FUNCTION_FAILURE_HANDLE(ret,(ret!=0),mess_matrix_init);
    ...
    ret = mess_equation_lyap(eqn, adiopt, A, E, B);     FUNCTION_FAILURE_HANDLE(ret, (ret!=0), mess_equation_lyap);
    ...
    ret = mess_lrcfadi_adi(eqn , adiopt, status->statC, ZCT); FUNCTION_FAILURE_HANDLE(ret, (ret!=0), mess_lrcfadi_adi);

    mess_equation_clear(&eqn);
    adiopt->type = old_op;
    /* adiopt->E = E;
       ret =mess_lrcfadi_lrcfadidual(A,B,C,ZB,ZCT,adiopt, status->statB,status->statC);
       FUNCTION_FAILURE_HANDLE(ret,(ret!=0), mess_lrcfadi_lrcfadidual);
       adiopt->E = NULL; */
    te = mess_wtime();
    ...
    ret = mess_matrix_multiply(MESS_OP_HERMITIAN, E, MESS_OP_NONE, ZCT, ZC);
    FUNCTION_FAILURE_HANDLE(ret,(ret!=0), mess_matrix_multiply);
    mess_matrix_clear(&ZCT);

    /*-----------------------------------------------------------------------------
     *  compute the SVD
     *-----------------------------------------------------------------------------*/
    ts = mess_wtime();
    ret = mess_matrix_multiply(MESS_OP_HERMITIAN, ZC, MESS_OP_NONE, ZB, ZCZB);
    FUNCTION_FAILURE_HANDLE(ret, (ret!=0), mess_matrix_multiply);
    ...
    ret = mess_vector_alloc(SIGMA, (ZCZB->rows < ZCZB->cols)? ZCZB->rows:ZCZB->cols, MESS_REAL);
    FUNCTION_FAILURE_HANDLE(ret,(ret!=0), mess_vector_alloc);

    ret = mess_eigen_svd_econ(ZCZB,SIGMA, UC, UB);
    FUNCTION_FAILURE_HANDLE(ret,(ret!=0), mess_eigen_svd);
    ...
    // MSG_PRINT("call choose order\n");
    ret  = chooseorder(SIGMA, &tol, &maxr,A->rows,btopt->chooseorder_aux);
    ...
    ret = mess_matrix_multiply(MESS_OP_NONE, ZC, MESS_OP_NONE, T, SC);      FUNCTION_FAILURE_HANDLE(ret, (ret!=0), mess_matrix_multiply);
    ret = mess_matrix_backslashm(MESS_OP_TRANSPOSE, E,SC,W);                FUNCTION_FAILURE_HANDLE(ret, (ret!=0), mess_matrix_backslashm);
    te = mess_wtime();

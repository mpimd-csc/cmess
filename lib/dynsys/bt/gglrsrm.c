...
/**
 * @brief Generalized low-rank square root balanced truncation for first order GLTI to GLTI systems.
 ...
 *
 * The @ref mess_bt_gglrsrm function computes the left and the right projection matrix for
 * a balanced truncation reduced order model of a generalized state space system to a
 * generalized state space system. \n
 * In order to get the reduced order GLTI model from the output, you have to call the
 * \ref mess_dynsys_project_to_glti function with the computed \f$ V \f$ and \f$ W \f$. \n
 ...
 * If you do not want to choose the reduced order in an adaptive way you can specify a \b chooseorder
 * function to decide it in your own way. The default choose order function is \ref mess_bt_chooseorder_default.
 *
 * The algorithm is based on Algorithm 7.3 in \cite Saa09 .
 *
 *
 */
int mess_bt_gglrsrm(mess_dynsys sys, mess_bt_options btopt, mess_options adiopt, mess_matrix V, mess_matrix W, mess_bt_status status) {
    MSG_FNAME(__func__);
    mess_matrix A,B,C,E, ZB,ZC,ZBT, ZCZB, SC;
    ...
    ret = mess_matrix_init(&ZBT); FUNCTION_FAILURE_HANDLE(ret,(ret!=0),mess_matrix_init);
    ...
    ret = mess_lrcfadi_adi(eqn , adiopt, status->statB, ZBT); FUNCTION_FAILURE_HANDLE(ret, (ret!=0), mess_lrcfadi_adi);
    ...
    adiopt->type = old_op;
    /* adiopt->E = E;
       ret =mess_lrcfadi_lrcfadidual(A,B,C,ZBT,ZC,adiopt, status->statB,status->statC);
    ...
    ret = mess_matrix_multiply(MESS_OP_NONE, E, MESS_OP_NONE, ZBT, ZB);
    FUNCTION_FAILURE_HANDLE(ret,(ret!=0), mess_matrix_mul);
    // mess_matrix_clear(&ZBT);
    ...
    ret = mess_matrix_multiply(MESS_OP_NONE, ZBT, MESS_OP_NONE, T, V);      FUNCTION_FAILURE_HANDLE(ret,(ret!=0), mess_matrix_multiply);
    ...
    // ret = mess_matrix_backslashmt(E,SC,W); FUNCTION_FAILURE_HANDLE(ret,(ret!=0), mess_matrix_backslashmt);
    te = mess_wtime();

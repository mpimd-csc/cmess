...
/**
 * @brief Low-rank square root balanced truncation for first order LTI systems.
 ...
 *
 * The @ref mess_bt_lrsrm function computes the left and the right projection matrix for
 * a balanced truncation reduced order model. \n
 * In order to get the reduced order model from the output, you have to call the \ref mess_dynsys_project_to_lti
 * function with the computed  \f$ V \f$ and \f$ W \f$. \n
 ...
 *
 * The algorithm is based on Algorithm 7.1 in \cite Saa09 .
 *
 *
 */
int mess_bt_lrsrm(mess_dynsys sys, mess_bt_options btopt, mess_options adiopt, mess_matrix V, mess_matrix W, mess_bt_status status) {
    ...
    if ( !MESS_IS_DYNSYS_LTI(sys) ){
        MSG_ERROR("Only for Standard space LTI systems\n");
    ...
    ret = mess_equation_lyap(eqn, adiopt, A, NULL, B);  FUNCTION_FAILURE_HANDLE(ret, (ret!=0), mess_equation_lyap);
    ...
    /* ret =mess_lrcfadi_lrcfadidual(A,B,C,ZB,ZC,adiopt, status->statB,status->statC);
    ...
    FUNCTION_FAILURE_HANDLE(ret,(ret!=0), mess_vector_init);

...
/**
 * @file lib/lrcf_adi/equation_glyap_so1.c
 ...
 *
 * The @ref __glyap_so1 structure contains all matrices, solvers and additional data for Second Order System function handles.
...
typedef struct {
    ...
    mess_int_t shifts;      /**< Number of direct solvers in @ref __glyap_so1.shiftsolver. */
...
/**
 * @internal
 * @brief Multiply with operator \f$ \begin{bmatrix} 0 & -K \\ -K & -D \end{bmatrix} \f$ to a @ref mess_matrix.
 ...
 *
 * The @ref AX_apply function computes \f$ out = \begin{bmatrix} 0 & -K \\ -K & -D \end{bmatrix}  in\f$.
 *
 * @attention Internal use only.
 */
static int AX_apply(mess_equation e, mess_operation_t op, mess_matrix in, mess_matrix out) {
    ...
    __glyap_so1 *eqn = (__glyap_so1 *) e->aux;
    ...
    /*-----------------------------------------------------------------------------
     *  perform [-K*x2;-K*x1-D*x2]->out
     *-----------------------------------------------------------------------------*/
    mess_matrix temp1, temp2, temp3, temp4;
    ...
    ret = mess_matrix_init(&temp4);                                                 FUNCTION_FAILURE_HANDLE(ret,(ret!=0),mess_matrix_init);

    //take upper part of in
    ret = mess_matrix_rowsub(in, 0,n-1, temp3);                                     FUNCTION_FAILURE_HANDLE(ret,(ret!=0),mess_matrix_rowsub);

    //take lower part of in
    ret = mess_matrix_rowsub(in, n, 2*n-1, temp4);                                  FUNCTION_FAILURE_HANDLE(ret,(ret!=0),mess_matrix_rowsub);

    ret = mess_matrix_multiply(op, eqn->K, MESS_OP_NONE, temp4, temp1);             FUNCTION_FAILURE_HANDLE(ret,(ret!=0),mess_matrix_mul);
    ret = mess_matrix_multiply(op, eqn->D, MESS_OP_NONE, temp4, temp2);             FUNCTION_FAILURE_HANDLE(ret,(ret!=0),mess_matrix_mul);
    ret = mess_matrix_multiply(op, eqn->K, MESS_OP_NONE, temp3, temp4);             FUNCTION_FAILURE_HANDLE(ret,(ret!=0),mess_matrix_mul);

    ret = mess_matrix_add(-1.0,temp2,-1.0,temp4);                                   FUNCTION_FAILURE_HANDLE(ret,(ret!=0),mess_matrix_mul);
    ret = mess_matrix_scale(-1.0, temp1);                                           FUNCTION_FAILURE_HANDLE(ret,(ret!=0),mess_matrix_scale);
    ret = mess_matrix_cat(temp1,NULL,temp4,NULL,MESS_DENSE,out);                    FUNCTION_FAILURE_HANDLE(ret,(ret!=0),mess_matrix_cat);

    /*-----------------------------------------------------------------------------
     *  clear additional memory
     *-----------------------------------------------------------------------------*/
    MESS_CLEAR_MATRICES(&temp1,&temp2,&temp3,&temp4);
...
/**
 * @internal
 * @brief Multiply with operator \f$ \begin{bmatrix} -K & 0 \\ 0 & M \end{bmatrix} \f$ to a @ref mess_matrix.
 * @param[in] e         input @ref mess_equation structure
 * @param[in] op        input operation applied to block matrix
 * @param[in] in        input matrix \f$in\f$
 * @param[out] out      output matrix \f$out\f$
 * @return zero on succes or a non-zero error value otherwise
 *
 * The @ref EX_apply function computes \f$ out = \begin{bmatrix} -K & 0 \\ 0 & M\end{bmatrix}  in\f$.
 *
 * @attention Internal use only.
 */
static int EX_apply(mess_equation e, mess_operation_t op, mess_matrix in, mess_matrix out) {
    ...
    //take lower part of in
    ret = mess_matrix_rowsub(in, n, 2*n-1, temp2);                                      FUNCTION_FAILURE_HANDLE(ret,(ret!=0),mess_matrix_rowsub);

    ret = mess_matrix_multiply(op, eqn->K, MESS_OP_NONE, temp1, temp3);                 FUNCTION_FAILURE_HANDLE(ret,(ret!=0),mess_matrix_mul);
    ret = mess_matrix_multiply(op, eqn->M, MESS_OP_NONE, temp2, temp1);                 FUNCTION_FAILURE_HANDLE(ret,(ret!=0),mess_matrix_mul);

    ret = mess_matrix_scale(-1.0, temp3);                                               FUNCTION_FAILURE_HANDLE(ret,(ret!=0),mess_matrix_scale);

    ret = mess_matrix_cat(temp3,NULL,temp1,NULL,MESS_DENSE,out);                        FUNCTION_FAILURE_HANDLE(ret,(ret!=0),mess_matrix_cat);

    /*-----------------------------------------------------------------------------
     *  clear additional memory
     *-----------------------------------------------------------------------------*/
    MESS_CLEAR_MATRICES(&temp1,&temp2,&temp3);
...
/**
 * @internal
 * @brief Solve with operator \f$ \begin{bmatrix} 0 & -K \\ -K & -D \end{bmatrix} \f$ to a @ref mess_matrix.
 * @param[in] e         input @ref mess_equation structure
 * @param[in] op        input operation applied to block matrix
 * @param[in] in        input matrix \f$in\f$
 * @param[out] out      output matrix \f$out\f$
 * @return zero on succes or a non-zero error value otherwise
 *
 * The @ref AINV_apply function solves the system \f$ \begin{bmatrix} 0 & -K \\ -K & -D \end{bmatrix} out=in \f$
 *  via the equivalent formulation
 * \f[
 *  \begin{aligned}
 *      out(n+1:end,:)  &= -K^{-1} in(1:n,:) \\
...
static int AINV_apply(mess_equation e, mess_operation_t op, mess_matrix in, mess_matrix out){
    ...
    /*-----------------------------------------------------------------------------
     *  perform [-K\x1;-K\(b+D*x2)]->out
     *-----------------------------------------------------------------------------*/
    mess_matrix f1, f2, in1,in2;
    MESS_INIT_MATRICES(&in1,&in2,&f1,&f2);

    ret = mess_matrix_rowsub(in,0,n-1,in1);                                     FUNCTION_FAILURE_HANDLE(ret,(ret!=0),mess_matrix_rowsub);
    ret = mess_matrix_rowsub(in,n,2*n-1,in2);                                   FUNCTION_FAILURE_HANDLE(ret,(ret!=0),mess_matrix_rowsub);

    //K is a full rank Matrix first solve with K
    ret = mess_direct_solvem(op, eqn->Ksolver, in1, f2);                            FUNCTION_FAILURE_HANDLE(ret,(ret!=0),mess_direct_solve);
    ret = mess_matrix_scale(-1.0,f2);                                           FUNCTION_FAILURE_HANDLE(ret,(ret!=0),mess_vector_scale);

    //Dx2
    ret = mess_matrix_multiply(op, eqn->D, MESS_OP_NONE, f2, in1);              FUNCTION_FAILURE_HANDLE(ret,(ret!=0),mess_matrix_mul);
    ret = mess_matrix_add(1.0, in1, 1.0, in2);                                  FUNCTION_FAILURE_HANDLE(ret,(ret!=0),mess_matrix_add);
    ret = mess_direct_solvem(op, eqn->Ksolver,in2, f1 );                        FUNCTION_FAILURE_HANDLE(ret,(ret!=0),mess_direct_solve);
    ret = mess_matrix_scale(-1.0,f1);                                           FUNCTION_FAILURE_HANDLE(ret,(ret!=0),mess_matrix_scale);
    ret = mess_matrix_cat(f1, NULL, f2, NULL, MESS_DENSE, out);                 FUNCTION_FAILURE_HANDLE(ret,(ret!=0),mess_matrix_cat);

    /*-----------------------------------------------------------------------------
     *  clear additional memory
     *-----------------------------------------------------------------------------*/
    MESS_CLEAR_MATRICES(&f1,&f2,&in1,&in2);
...
/**
 * @internal
 * @brief Solve with \f$ \begin{bmatrix} -K & 0 \\ 0 & M \end{bmatrix} \f$.
 * @param[in] e         input @ref mess_equation structure
 * @param[in] op        input operation applied to block matrix
 * @param[in] in        input matrix \f$in\f$
 * @param[out] out      output matrix \f$out\f$
 * @return zero on succes or a non-zero error value otherwise
 *
 * The @ref EINV_apply function solves the system \f$ \begin{bmatrix} -K & 0 \\ 0 & M\end{bmatrix}out=  in\f$ via
 *
 * \f[
 *  \begin{aligned}
 *      out(1:n,:) &= -K^{-1}(in(1:n,:))    \\
...
static int EINV_apply(mess_equation e, mess_operation_t op, mess_matrix in , mess_matrix out){
     ...
     *-----------------------------------------------------------------------------*/
    mess_matrix temp1, temp2, temp3;
    MESS_INIT_MATRICES(&temp1,&temp2,&temp3);

    ret = mess_matrix_rowsub(in, 0, n-1, temp1);                                FUNCTION_FAILURE_HANDLE(ret,(ret!=0),mess_matrix_rowsub);
    ret = mess_matrix_rowsub(in, n, n2-1, temp2);                               FUNCTION_FAILURE_HANDLE(ret,(ret!=0),mess_matrix_rowsub);
    ret = mess_direct_solvem(op, eqn->Ksolver, temp1, temp3);                   FUNCTION_FAILURE_HANDLE(ret,(ret!=0),mess_direct_solve);
    ret = mess_matrix_scale(-1.0,temp3);                                            FUNCTION_FAILURE_HANDLE(ret,(ret!=0),mess_matrix_scale);
    ret = mess_direct_solvem(op,eqn->Msolver, temp2, temp1);                    FUNCTION_FAILURE_HANDLE(ret,(ret!=0),mess_direct_solvem);
    ret = mess_matrix_cat(temp3, NULL, temp1, NULL, MESS_DENSE, out);           FUNCTION_FAILURE_HANDLE(ret,(ret!=0),mess_matrix_cat);
...
/**
 * @internal
 * @brief Generate a shifted solver for the system matrix \f$ \begin{bmatrix} -pK & -K \\ -K & -D + pM \end{bmatrix}\f$.
 * @param[in] e             input @ref mess_equation structure
 ...
 * \f[
 *      \begin{bmatrix} -pK & -K \\ -K & -D + pM \end{bmatrix}.
 * \f]
...
/**
 * @internal
 * @brief Solves shifted linear block system matrix \f$ \begin{bmatrix} -pK & -K \\ -K & -D + pM \end{bmatrix}\f$.
 ...
 * \f[
 *      op\left(\begin{bmatrix} -pK & -K \\ -K & -D + pM \end{bmatrix}\right) out = in
 * \f]
 * via
 *
 * \f[
 *  \begin{aligned}
 *      -p K out(1:n,:) &= in(1:n,:) + K out(n+1:end,:) \\
 *      (K-pD+p^2M)out(n+1:end,:) &= p in(n+1:end,:) - in(1:n,:)
 *  \end{aligned}
 * \f]
 *
 * @attention Internal use only.
...
static int ApEINV_apply (mess_equation e, mess_operation_t op, mess_double_cpx_t p, mess_int_t idx_p, mess_matrix in, mess_matrix out) {
    ...
    /*-----------------------------------------------------------------------------
     *  perform -p*K\(b1+K*x2)->f1 and (K-p*D+p^2*M)\(p*b2-b1)->f2
     *-----------------------------------------------------------------------------*/
    mess_matrix f2, f1, rhs, x1,x2;
    ...
    ret = mess_matrix_rowsub(in,0,n-1,f1);                                          FUNCTION_FAILURE_HANDLE(ret,(ret!=0),mess_matrix_rowsub);
    ret = mess_matrix_rowsub(in,n,2*n-1,f2);                                        FUNCTION_FAILURE_HANDLE(ret,(ret!=0),mess_matrix_rowsub);
    ret = mess_matrix_copy(f1,rhs);                                                 FUNCTION_FAILURE_HANDLE(ret,(ret!=0),mess_matrix_copy);
    ret = mess_matrix_addc(op==MESS_OP_HERMITIAN?conj(p):p,f2,-1.0,rhs);            FUNCTION_FAILURE_HANDLE(ret,(ret!=0),mess_matrix_addc);
    ret = mess_direct_solvem(op,eqn->shiftsolver[idx_p],rhs,x2);                    FUNCTION_FAILURE_HANDLE(ret,(ret!=0),mess_direct_solvem);
    ret = mess_matrix_copy(x2,f2);                                                 FUNCTION_FAILURE_HANDLE(ret,(ret!=0),mess_matrix_copy);
    ret = mess_direct_solvem(op,eqn->Ksolver,f1,rhs);                               FUNCTION_FAILURE_HANDLE(ret,(ret!=0),mess_direct_solvem);
    ret = mess_matrix_addc(1.0,rhs,1.0,f2);                                         FUNCTION_FAILURE_HANDLE(ret,(ret!=0),mess_matrix_addc);
    ret = mess_matrix_scalec(op==MESS_OP_HERMITIAN?1.0/(-conj(p)):1.0/(-p),f2);     FUNCTION_FAILURE_HANDLE(ret,(ret!=0),mess_direct_scalc);
    ret = mess_matrix_cat(f2,NULL,x2,NULL,MESS_DENSE,out);                          FUNCTION_FAILURE_HANDLE(ret,(ret!=0),mess_matrix_cat);
 ...
 * @return zero on succes or a non-zero error value otherwise
 *
 * The @ref ApEINV_clear clear generated solvers by using @ref mess_direct_clear the fields of @ref __glyap_so1.shiftsolver are cleared.
 *
 * @attention Internal use only.
 *
 */
...
static int parameter(mess_equation e, mess_options opt, mess_status stat){
    ...
    mess_int_t i,j=0;
    //mess_vector_print(opt->adi_shifts_p);
...
/**
 * @internal
 * @brief Clear additional allocated field of @ref __glyap_so1 structure.
 * @param[in, out] e         input @ref mess_equation structure
 * @return zero on succes or a non-zero error value otherwise
 *
 ...
 * |:----------------------------------------:|:-----------------------------------------------------------------------------------------------:|
 * | @ref __glyap_so1.Ksolver                 | \f$ K^{-1} \f$                                                          |
 * | @ref __glyap_so1.Msolver                 | \f$ M^{-1} \f$                                                      |
 * </center>.
 *
 * @attention Internal use only.
 */
...
/**
 * @brief Generate an equation object to a Riccati Equation arising from a second order system.
 ...
 *
 * The @ref mess_equation_griccati_so1 function creates an equation object from a second order LTI system
 * \f[
 *  \begin{array}{cccc}
 ...
 * \end{array}
 * \f]
 * The Riccati solver \ref mess_lrcfadi_nm will then solve on
 ...
 *
 * If @ref mess_options.type \ref MESS_OP_NONE
 * \f[
 *  \begin{array}{cccc}
...
* \f[
    ...
    *  \left[
        ...
        *  & \in \mathbb{R}^{2n\times t} , \\
            *  \mathcal{C} &=& C &\in \mathbb{R}^{s \times n}
            *  \end{array}
            * \f]
            *
            ...
            */
            int mess_equation_griccati_so1(mess_equation e, mess_options opt , mess_matrix M, mess_matrix D, mess_matrix K, mess_matrix B, mess_matrix C, double lowerbound, double upperbound){
                ...
                if(lowerbound>upperbound){
                    MSG_ERROR("lowerbound is higher than upperbound\n");
                    return  MESS_ERROR_ARGUMENTS;
                }
                 ...
                 *  Setup the equation
                 *-----------------------------------------------------------------------------*/
                mess_try_alloc(data,__glyap_so1 *, sizeof(__glyap_so1));
                data->M         = M;
                data->D         = D,
                    data->K         = K;
                ...
                e->ApEX.generate    = NULL;//ApEX_generate ;
                e->ApEX.apply       = NULL; //ApEX_apply;
                e->ApEX.clear       = NULL;//ApEX_clear;
...
/**
 * @brief Generate an Equation object to a Lyapunov Equation arising from a second order system.
 * @param[out] e  equation object to setup
 * @param[in]  opt  input options for the ADI iteration
 * @param[in]  M      input \f$ M \f$ matrix of the LTI system
 * @param[in]  D      input \f$ D \f$ matrix of the LTI system
 ...
 * @return zero on success or a non zero error code otherwise
 *
 * The @ref mess_equation_glyap_so1 function creates an equation object from a second order LTI system
 * \f[
 * \begin{array}{ccc}
 *     M\ddot{x} + D\dot{x} + K x &=& B u
 * \end{array}, \f]
 * where \f$ M \f$, \f$ D \f$ and \f$ K \f$ are  matrices of dimension \f$ n \f$.\n
 * Furthermore \f$ K \f$ need to be invertible and \f$ M \f$ positive definite. \n
 * The system is implicitly transformed to its first order representation:
 *  \f[
 *    \underbrace{\left[

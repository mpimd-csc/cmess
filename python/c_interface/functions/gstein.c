 ...
 * @file python/c_interface/functions/gstein.c
 * @brief Interface to @ref mess_gstein.
 *
 * @author @mbehr
 *
 */
...
/**
 * @brief Interface to \ref mess_gstein for use with @python.
 * @param[in]  self input
 * @param[in]  args input
 * @return @python object containing X and optional the schur decomposition or None in case of an error / exception.
 *
 * The @ref pymess_gstein function provides an interface to \ref mess_gstein function  in order
...
PyObject* pymess_gstein(PyObject * self, PyObject * args){
     ...
     *-----------------------------------------------------------------------------*/
    if(schur_given){
        if(mess_tgstein(op, Ahat, QA, Ehat, QE, Y, X)){
            PyErr_SetString(PyExc_RuntimeError,"C-M.E.S.S.: mess_tgstein returned an error.");
            goto error;
        }
    }else{
        if(mess_gstein(op, A, E, Y, Ahat, QA, Ehat, QE, X)){
            PyErr_SetString(PyExc_RuntimeError,"C-M.E.S.S.: mess_gstein returned an error.");
            goto error;
        }
    }

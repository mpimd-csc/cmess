 ...
 * @file python/c_interface/functions/lradi.c
 * @brief Interface to @ref mess_lradi.
 *
 * @author @koehlerm
 * @author @nitin
 * @author @mbehr
...
PyObject* pymess_lradi(PyObject * self, PyObject * args) {
     ...
     *-----------------------------------------------------------------------------*/
    eqn = mess_equation_from_python(pyEqn, mem, MESS_EQN_GLYAP, &need_callback);
    if (!eqn) {
    ...
    if ( !opt ) {
        PyErr_SetString(PyExc_RuntimeError, "Cannot copy the options from Python.");
        goto error;
    }
    ...
    if(need_callback){
        ret = mess_lradi(eqn, opt, status, Z);
    } else {
    ...
    if(ret){
        PyErr_SetString(PyExc_RuntimeError,"C-M.E.S.S.: mess_lradi returned an error.");
        goto error;
    }

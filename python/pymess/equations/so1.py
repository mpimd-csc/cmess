...
r"""Interface to mess_equation_glyap_so1 and mess_equation_griccati_so1."""
from pymess.equation import Equation

class EquationGLyapSO1(Equation):
    r"""Interface to mess_equation_glyap_so1.
    ...
    .. math::
        ...
        \underbrace{\begin{bmatrix} -K & 0 \\ 0 & M \end{bmatrix}}_{\mathcal{E}}
        \begin{bmatrix} \dot{x} \\ \ddot{x} \end{bmatrix}
        =
        \underbrace{\begin{bmatrix} 0 & -K \\ -K & -D  \end{bmatrix}}_{\mathcal{A}}
        \begin{bmatrix} x \\ \dot{x} \end{bmatrix}
        +
        \underbrace{\begin{bmatrix} 0 \\ B \end{bmatrix}}_{\mathcal{B}}.

    Therefore an instance of :class:`.EquationGLyapSO1` represents
    the generalized Lyapunov equations:

    .. math::

            \mathcal{A} X \mathcal{E}^T + \mathcal{E} X\mathcal{A}^T + \mathcal{B}\mathcal{B}^T &= 0 \\
            \mathcal{A}^T X\mathcal{E}  +\mathcal{E}^T X \mathcal{A} + \mathcal{B}^T\mathcal{B} &= 0
    ...
    d: (n,n) sparse real matrix
        Matrix :math:`A` of second order system.
    ...
    Examples
    --------
    Further examples :download:`pymess_lradi_so1.py <../../example/pymess_lradi_so1.py>`

    """

    def __init__(self, opt, m, d, k, b, lowerbound, upperbound):

        super(EquationGLyapSO1, self).__init__("EquationGLyapSO1", opt, m.shape[0])
        self.m = m
        self.d = d
        self.k = k
        self.b = b
...
class EquationGRiccatiSO1(Equation):
    r"""Interface to mess_equation_griccati_so1.
    ...
    .. math::
        ...
        \mathcal{E}^T X\mathcal{B}\mathcal{B}^T X \mathcal{E} +
    ...
    Examples
    --------
    Further examples :download:`pymess_lrnm_so1.py <../../example/pymess_lrnm_so1.py>`

    """

    def __init__(self, opt, m, d, k, b, c, lowerbound, upperbound):

        super(EquationGRiccatiSO1, self).__init__("EquationGRiccatiSO1", opt, m.shape[0])
        self.m = m
        self.d = d
        self.k = k
        self.b = b

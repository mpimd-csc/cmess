 ...
 * @{
 *
 * @file tests/lrcf_adi/check_so2_handles.c
 * @brief Check function handles for second order systems (so2).
 * @author  @mbehr
 * @test
 * This function uses the @ref mess_equation_glyap_so2 function defined in equation_glyap_so2.c that means it
 ...
 * and checks if the resulting equation object is solved correctly by function handles defined in equation_glyap_so2.
 *
 * @}
 *
 */
...
int main ( int argc, char ** argv) {
    ...
    double diff=.0, eps=1e-10;
    mess_vector x1, x2, y1, y2, shifts;
    mess_matrix M, D, K, B, fullA, fullM, X1, X2, Y1, Y2, Tmp, Eye;
    mess_equation eqn;
    mess_options opt;
    ...
    //CALL(mess_matrix_rand(M,100,100,MESS_CSR,0.8));
    //CALL(mess_matrix_rand(D,100,100,MESS_CSR,0.8));
    //CALL(mess_matrix_rand(K,100,100,MESS_CSR,0.8));
    ...
    CALL(mess_equation_glyap_so2(eqn,opt,M,D,K,Tmp,1e-6,1e+6));


    //build fullA=[0,I;-K,-D]
    CALL(mess_matrix_eye(Eye,n,n,MESS_CSR));
    CALL(mess_matrix_scale(-1,Eye));
    CALL(mess_matrix_cat(NULL,Eye,K,D,MESS_CSR,fullA));
    CALL(mess_matrix_scale(-1,fullA));

    //build fullE=[I,0;0,M]
    CALL(mess_matrix_scale(-1,Eye));
    CALL(mess_matrix_cat(Eye,NULL,NULL,M,MESS_CSR,fullM));
    ...
    for(j=0;j<2;++j){
        for(i=0;i<3;++i){
            ...
            if(j==0){
                ...
                if(eqn->ApEINV.generate){
                    CALL(eqn->ApEINV.generate(eqn,shifts));//dummy shiftvector
                }
                ...
                for(jj=0;jj<shifts->dim;++jj){shifts->values_cpx[jj]*=(1+I)*(jj%3 +1)*((jj%2)?1:-1);};
                if(eqn->ApEINV.generate){
                    CALL(eqn->ApEINV.generate(eqn,shifts));//dummy shiftvector
                }
                for(jj=0;jj<shifts->dim;++jj){

 ...
 * @{
 * @file tests/eigen/check_arpack_lanczos.c
 * @brief Check the solution of the eigenvalue problem using the Lanczos process from @arpack.
 * @test
 * This function checks the @ref mess_eigen_arpack_lanczos and
 * @ref mess_eigen_arpack_lanczos_template functions defined in
 * arpack.c that means it checks if the eigenvalue problem \f[ Ax = \lambda x \f]
 * is solved correctly for a given matrix \f$ A \f$ using the Lanczos process from @arpack.
 *
 * @}
 */
 ...
 * @param[in] data   input data of a matrix
 * @param[in] x      input vector
 * @param[out] y        output vectorvector
 * @return zero on success or a non-zero error value otherwise
 *
 * The @ref mvp function determines a matrix \f$ A \f$ out of \f$ data \f$ and computes the matrix-vector product
 * \f[ y=Ax. \f]
 *
 */
 ...
 * @param[in] which      input eigenvalues to select in the Lanczos process
 * @return zero on success or a non-zero error value otherwise
 *
 * The @ref call_lanczos function computes the eigenvalues and eigenvector for the eigenvalue problem
 * \f[ A x= \lambda x\f]
 * and checks if the eigenvalue problem is fulfilled.
 *
 */
int call_lanczos(mess_matrix A, mess_int_t nev, mess_eigen_arpack_which_t which) {
    mess_vector ev;
    mess_matrix V;
    ...
    CALL(mess_eigen_arpack_lanczos_template( mvpcall , nev, opt, ev,V));
    ...
    CALL(mess_eigen_arpack_lanczos( A, nev, opt, ev,V));
    mess_vector_print(ev);
    err += check_ev(A,ev,V);
...
int main (int argc, char *argv[])
{
    ...
    CALL(call_lanczos(matrix,nev,MESS_EIGEN_ARPACK_LARGE_MAGNITUDE));

    printf("Check MESS_EIGEN_ARPACK_SMALL_MAGNITUDE:\n:");
    CALL(call_lanczos(matrix,nev,MESS_EIGEN_ARPACK_SMALL_MAGNITUDE));

    printf("Check MESS_EIGEN_ARPACK_LARGE_ALGEBRAIC:\n:");
    CALL(call_lanczos(matrix,nev,MESS_EIGEN_ARPACK_LARGE_ALGEBRAIC));

    printf("Check MESS_EIGEN_ARPACK_SMALL_ALGEBRAIC:\n:");
    CALL(call_lanczos(matrix,nev,MESS_EIGEN_ARPACK_SMALL_ALGEBRAIC));

    printf("Check MESS_EIGEN_ARPACK_BOTH_ENDS:\n:");
    CALL(call_lanczos(matrix,nev,MESS_EIGEN_ARPACK_BOTH_ENDS));

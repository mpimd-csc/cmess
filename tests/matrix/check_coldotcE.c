 ...
 * @{
 * @file tests/matrix/check_coldotcE.c
 * @brief Check the \ref mess_matrix_coldotcE function.
 * @author  @mbehr
 * @test
 * This function check the @ref mess_matrix_coldotcE function defined in colops.c, that means it checks if the dot product
 * \f[Q(:,col_1)^H*E*Q(:,col_2) \f]
 * is computed correctly for a given complex matrix \f$ Q \f$ and a
...
#define CHECKCOLDOTCE(Q,E,COL1,COL2,SOL,DOT,ERR,EPS,DIFF){                  \
    CALL(mess_matrix_coldotcE(Q, E, COL1, COL2, &DOT ))                 \
    DIFF=cabs(SOL->values_cpx[COL1*SOL->ld+COL2]-DOT);                  \
    if(DIFF>EPS){++ERR;                                                 \
        printf("Dot: %e\t + I*%e\n",creal(DOT), cimag(DOT));                \
        printf("Diff%e\n\n",DIFF);                                          \
    }                                                                       \
...
int main ( int argc, char ** argv) {
    ...
    double eps = 1e-12;
    double diff=0,diagonaladd=0;    //needed for creating a symmetric positiv definit Matrix E
    mess_double_cpx_t dot=0;
    ...
    CALL(mess_matrix_tocomplex(Q));
    CALL(mess_matrix_scalec(1+I,Q));
    CALL(mess_matrix_convert(Q,Qcsr,MESS_CSR));
    CALL(mess_matrix_convert(Q,Qcsc,MESS_CSC));

    //create a positiv definite matrix <-> symmetric diagonaldominant matrix
    ...
    mess_matrix_multiply(MESS_OP_HERMITIAN, Q, MESS_OP_NONE, E, temp);
    ...
    for(col1=0;col1<cols;++col1){
        for(col2=0;col2<cols;++col2){
            CHECKCOLDOTCE(Q,E,col1,col2,sol,dot,err,eps,diff);
            CHECKCOLDOTCE(Qcsc,E,col1,col2,sol,dot,err,eps,diff);
            CHECKCOLDOTCE(Qcsr,E,col1,col2,sol,dot,err,eps,diff);
            CHECKCOLDOTCE(Q,Ecsc,col1,col2,sol,dot,err,eps,diff);
            CHECKCOLDOTCE(Qcsc,Ecsc,col1,col2,sol,dot,err,eps,diff);
            CHECKCOLDOTCE(Qcsr,Ecsc,col1,col2,sol,dot,err,eps,diff);
            CHECKCOLDOTCE(Q,Ecsr,col1,col2,sol,dot,err,eps,diff);
            CHECKCOLDOTCE(Qcsc,Ecsr,col1,col2,sol,dot,err,eps,diff);
            CHECKCOLDOTCE(Qcsr,Ecsr,col1,col2,sol,dot,err,eps,diff);
        }
    }

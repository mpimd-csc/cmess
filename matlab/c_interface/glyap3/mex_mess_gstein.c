...
/**
 * @file matlab/c_interface/glyap3/mex_mess_gstein.c
 * @brief
 * @author @mbehr
 */
...
void mex_mess_gstein( int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]){
    ...
    if((nlhs == 1 || nlhs == 3 || nlhs == 5) && 3 == nrhs || (nrhs == 4 && mxIsClass(prhs[3], "mess_operation_t"))){
         ...
         *-----------------------------------------------------------------------------*/
        ret = mess_gstein(op, A, E, Y, Ahat, QA, Ehat, QE, X);

        if (ret){
            csc_error_message("C-M.E.S.S.: mess_gstein returned = %d.\n",ret);
            goto CLEAR;
        }
        ...
        if(nrhs == 4 || nrhs == 5){
             ...
             *-----------------------------------------------------------------------------*/
            ret = mess_tgstein(op, Ahat, QA, NULL, NULL, Y, X);

            if (ret){
                csc_error_message("C-M.E.S.S.: mess_tgstein returned = %d.\n",ret);
                goto CLEAR;
            }
        ...
        if(nrhs == 7 || nrhs == 8){
             ...
             *  perform solve and using schur decomposition
             *-----------------------------------------------------------------------------*/
            ret = mess_tgstein(op, Ahat, QA, Ehat, QE, Y, X);

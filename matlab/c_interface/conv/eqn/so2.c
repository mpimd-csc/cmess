...
/**
 * @file matlab/c_interface/conv/eqn/so2.c
...
mess_equation eqn_conv_lyap_so2(const mxArray* m_eqn, const mxArray* m_opt,  mess_freelist mem) {
    ...
    if (!mxIsClass(m_eqn, "mess_equation_glyap_so2")){
        csc_error_message("Expected mess_equation_glyap_so2 class.\nAn error occured return NULL Pointer.");
        return NULL;
    }
    ...
    ret = mess_equation_glyap_so2(eqn, opt, M, D, K, B, lowerbound, upperbound);
...
mess_equation eqn_conv_ric_so2(const mxArray* m_eqn, const mxArray* m_opt,  mess_freelist mem) {
    ...
    if (!mxIsClass(m_eqn, "mess_equation_griccati_so2")){
        csc_error_message("Expected mess_equation_griccati_so2 class.\nAn error occured return NULL Pointer.");
        return NULL;
    }
    ...
    mess_equation eqn;
    mess_equation_init(&eqn);
    ret = mess_equation_griccati_so2(eqn, opt, M, D, K, B, C, lowerbound, upperbound);

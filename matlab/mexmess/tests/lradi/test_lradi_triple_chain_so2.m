% TEST_LRADI_TRIPLE_CHAIN_SO2 test for <a href="matlab:help mess_lradi">mess_lradi</a> using <a href="matlab:help mess_equation_glyap_so2">mess_equation_glyap_so2</a>.
%
%   TEST_LRADI_TRIPLE_CHAIN_SO2 tests the <a href="matlab:help mess_lradi">mess_lradi</a> solver.
%   TEST_LRADI_TRIPLE_CHAIN_SO2 creates a Lyapunov equation using <a href="matlab:help mess_equation_glyap_so2">mess_equation_glyap_so2</a> and solves it.
%
%   TEST_LRADI_TRIPLE_CHAIN_SO2 properties:
...
%
%   See also MESS_LRADI, MESS_EQUATION_GLYAP_SO2.
%
...
classdef test_lradi_triple_chain_so2 < matlab.unittest.TestCase
    ...
    methods
        function template_so2(test)
            ...
            if test.opt.type == mess_operation_t.MESS_OP_NONE
                % build equation
                eqn = mess_equation_glyap_so2(test.opt,test.M,test.D,test.K,test.B,test.lowerbound, test.upperbound);
            else
                eqn = mess_equation_glyap_so2(test.opt,test.M,test.D,test.K,test.C,test.lowerbound, test.upperbound);
            end
    ...
    methods(Test)
        ...
        function so2_lyapunov(test)
            test.opt.type = mess_operation_t.MESS_OP_NONE;
            test.template_so2();
        end

        function so2_dual_lyapunov(test)
